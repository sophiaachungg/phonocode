...
def build_label_map(gt_path: Path) -> Dict[Tuple[str, str], int]:
    ...
    # normalize target_word to match filename if needed
...
def main():
    ...
    # ---- load frozen WavLM encoder ----
    print(f"Loading WavLM encoder from: {MODEL_ID}")
    ...
    print("\n=== Training Logistic Regression on frozen WavLM embeddings ===")
    ...
    def eval_split(name, X_split, y_split):
        ...
        print(f"[{name}] Classification report:\n{classification_report(y_split, y_pred, digits=3)}")
    ...
    print("\n=== Training PyTorch MLP on frozen WavLM embeddings ===")

    # Optional: drop rare class 2 if you want pure binary classification
    # keep_idx = y_train != 2
    # X_train, y_train = X_train[keep_idx], y_train[keep_idx]
    # keep_idx = y_val != 2
    # X_val, y_val = X_val[keep_idx], y_val[keep_idx]
    # keep_idx = y_test != 2
    # X_test, y_test = X_test[keep_idx], y_test[keep_idx]

    num_classes = len(np.unique(np.concatenate([y_train, y_val, y_test])))
    assert set(np.unique(np.concatenate([y_train, y_val, y_test]))) <= {0, 1}, "Found non-binary labels"
    ...
    plt.tight_layout()
    plt.savefig("wavlm_mlp_learning_curves.png", dpi=150)
    print("Saved learning curves to wavlm_mlp_learning_curves.png")
